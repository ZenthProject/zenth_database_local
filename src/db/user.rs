use crate::db::crypto::{key_to_sqlcipher_pragma};
use crate::db::master::{MasterDb, UserEntry};
use crate::db::error::DbError;
use crate::db::time::current_timestamp;
...
impl UserDb {
    ...
    pub fn open_with_entry(entry: &UserEntry, password: &str) -> Result<Self, DbError> {
        ...
        let key = Argon2idHasher::derive_sqlcipher_key(password, &entry.salt)?;
    ...
    pub fn change_password(&self, old_password: &str, new_password: &str) -> Result<(), DbError> {
        ...
        let old_key = Argon2idHasher::derive_sqlcipher_key(old_password, &entry.salt)?;
        let old_pragma = key_to_sqlcipher_pragma(&old_key);
        Self::verify_key_unlocks_db(&self.db_path, &old_pragma)?;

        let new_key = Argon2idHasher::derive_sqlcipher_key(new_password, &entry.salt)?;
    ...
    fn verify_key_unlocks_db(db_path: &Path, pragma_key: &str) -> Result<(), DbError> {
